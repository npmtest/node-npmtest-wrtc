...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-wrtc/node_modules/wrtc/scripts/download-webrtc-libraries-and-headers.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header medium">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-wrtc/node_modules/wrtc/scripts/download-webrtc-libraries-and-headers.js</span></h1>
    <h2>
    ...
    <div class="path"><a href="../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-wrtc/node_modules/wrtc/scripts/</a> &#187; download-webrtc-libraries-and-headers.js</div>
</div>
<div class="body">
...
var options = {
  ...
  buildWebRTC: process.env.BUILD_WEBRTC ? <span class="branch-0 cbranch-no" title="branch not covered" >true </span>: defaults.buildWebRTC,
  ...
  skipDownload: process.env.SKIP_DOWNLOAD ? <span class="branch-0 cbranch-no" title="branch not covered" >true </span>: defaults.skipDownload
};
&nbsp;
/**
...
function downloadOrBuild(options) {
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (exists(path.join(modulePath, 'include')) &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >exists(path.join(modulePath, 'lib')))</span> {
<span class="cstat-no" title="statement not covered" >    return;</span>
  } else <span class="missing-if-branch" title="if path not taken" >I</span>if (options.skipDownload) {
<span class="cstat-no" title="statement not covered" >    return build(options);</span>
  }
...
<span class="cstat-no" title="statement not covered" >    console.log('Complete!');</span>
  }, <span class="fstat-no" title="function not covered" >function(error) {</span>
<span class="cstat-no" title="statement not covered" >    if (error.statusCode === 404) {</span>
<span class="cstat-no" title="statement not covered" >      if (options.buildWebRTC) {</span>
<span class="cstat-no" title="statement not covered" >        console.log('Binaries unavailable! Falling back to building from source.');</span>
<span class="cstat-no" title="statement not covered" >        build(options);</span>
<span class="cstat-no" title="statement not covered" >        return;</span>
      }
<span class="cstat-no" title="statement not covered" >      console.error('Binaries unavailable! Try building from source by setting BUILD_WEBRTC=1.');</span>
    } else {
 ...
 * @returns {undefined}
 */
<span class="fstat-no" title="function not covered" >function build(options) {</span>
<span class="cstat-no" title="statement not covered" >  var env = Object.assign({</span>
    OUT: computeModulePath(options.binary),
    WEBRTC_REF: options.commit
  }, process.env);
<span class="cstat-no" title="statement not covered" >  execSync('npm install ' + options.buildWebRTCDependency, {</span>
    env: env,
    stdio: 'inherit'
  });
}
